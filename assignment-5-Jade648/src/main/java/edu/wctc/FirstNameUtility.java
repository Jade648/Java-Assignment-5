...
private Map<String, Integer> firstNameCodeMap = new HashMap<>();
    ...
    public int encodeFirstName(String firstName, String middleInitial) throws MissingNameExeception {
if(firstName .equals("")){
throw new MissingNameExeception("firstName");

}
int a  = 0;
if(firstNameCodeMap.containsKey(firstName)){
    a += firstNameCodeMap.get(firstName);
}
else {
    a += firstInitialCodeMap.get(firstName.substring(0,1));
}

if(middleInitial.equals("")){
    a += middleInitialCodeMap.get(middleInitial);
}
return a;
    }
